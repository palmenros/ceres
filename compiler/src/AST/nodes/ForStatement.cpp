...
namespace Ceres::AST {
    ForStatement::ForStatement(const SourceSpan &sourceSpan,
        ...
        : Statement(sourceSpan), maybeInitDeclaration(std::move(maybeInitDeclaration)),
          maybeInitExpression(std::move(maybeInitExpression)),
          maybeConditionExpr(std::move(conditionExpr)), maybeUpdateExpr(std::move(updateExpr)),
          body(std::move(body)) {}
    ...
    std::vector<Node *> ForStatement::getChildren() const {
        ...
        if (maybeInitDeclaration != nullptr) { v.push_back(maybeInitDeclaration.get()); }

        if (maybeInitExpression != nullptr) { v.push_back(maybeInitExpression.get()); }

        if (maybeConditionExpr != nullptr) { v.push_back(maybeConditionExpr.get()); }

        if (maybeUpdateExpr != nullptr) { v.push_back(maybeUpdateExpr.get()); }
